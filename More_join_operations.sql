...
SELECT title, count (actorid) FROM movie JOIN casting ON (movie.id = casting.movieid) WHERE yr = 1978 GROUP BY title ORDER BY COUNT(casting.actorid) DESC, title;

#15) List all the people who have worked with 'Art Garfunkel'.

SELECT name FROM actor JOIN casting ON actor.id = casting.actorid WHERE casting.movieid IN
(SELECT movieid FROM casting JOIN actor ON actor.id = casting.actorid WHERE actor.name =  'Art Garfunkel')AND name != 'Art Garfunkel';