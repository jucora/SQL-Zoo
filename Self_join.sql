...
SELECT a.company, a.num FROM route a JOIN route b ON a.num = b.num AND a.company = b.company JOIN stops s1 ON s1.id = a.stop JOIN stops s2 ON s2.id = b.stop WHERE s1.name = 'Craiglockhart' AND s2.name = 'Tollcross';

#9) Give a distinct list of the stops which may be reached from 'Craiglockhart' by taking one bus, including 'Craiglockhart' itself, offered by the LRT company. Include the company and bus no. of the relevant services.

SELECT s2.name,  r1.company, r1.num FROM route r1 JOIN route r2 ON r1.company = r2.company AND r1.num = r2.num JOIN stops s1 ON s1.id = r1.stop JOIN stops s2 ON s2.id = r2.stop WHERE s1.name = 'Craiglockhart';
...
SELECT DISTINCT a.num, a.company, s2.name ,  c.num,  c.company
