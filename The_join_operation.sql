...
SELECT player, teamid, coach, gtime FROM goal JOIN eteam ON (eteam.id = goal.teamid) WHERE gtime <= 10;
...
SELECT teamname, COUNT(*) AS total_goals FROM eteam JOIN goal ON eteam.id = goal.teamid GROUP BY teamname;
...
SELECT matchid,mdate, COUNT(matchid) AS goals FROM game JOIN goal ON matchid = id WHERE team1 = 'POL' OR team2 = 'POL' GROUP BY matchid, mdate;
